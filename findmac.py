...
for dev_item in devices_list:
    ...
    for mac_item in mac_list:
        if (mac_item.findtext('l2ng-l2-mac-address') == sys.argv[1]) and (mac_item.findtext('l2ng-l2-mac-logical-interface') not in interface_list):
            interface_list.append(mac_item.findtext('l2ng-l2-mac-logical-interface'))
    if  interface_list!=[]:
        print("Device: %s" %dev.facts['hostname'])
        print("interfaces list: %s" % interface_list)
        print("")
    dev.close()
